<h1 align="center">
  
Blue Notebooks :blue_book:

</h1>
<div align="center">

[Documentation][wiki]&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;[Pull Requests][pr]&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;[Issues][issues]

![GitHub last commit][lastcommit] [![Pull Requests][img-pr-badge]][pr] [![License][img-license-badge]][license]

</div>

## About This Repository

This repository contains Jupyter Notebooks for analysis of Command-an-Control (C2 or C&C) communications across the network.

## Requirements and Installation

- Jupyter Notebooks and JupyterLab: [**link**](https://jupyter.org/install)

## Usage

Just spin up your JupyterLab and access it from your web browser.  
If you like using WSL and VisualStudio Code from Windows you can follow the documentation [**here**](https://code.visualstudio.com/blogs/2019/09/03/wsl2) and [**here**](https://code.visualstudio.com/docs/datascience/jupyter-notebooks).

## List of Notebooks

|Notebook||Purpose|
|-|-|
|dns.ipynb|Hunt C2 over DNS|

<!--Links-->
[issues]:https://github.com/jupitersinsight/bluenotebooks/issues "bluenotebooks Issues ➶"
[pull-requests]:https://github.com/jupitersinsight/bluenotebooks/pulls "bluenotebooks Requests ➶"
[wiki]:https://github.com/jupitersinsight/bluenotebooks/wiki "bluenotebooks Documentation ➶"
[repo]:https://github.com/jupitersinsight/bluenotebooks "bluenotebooks Repository ➶"
[pr]:https://github.com/jupitersinsight/bluenotebooks/pulls "bluenotebooks Pull Requests ➶"
[license]:https://github.com/jupitersinsight/bluenotebooks/blob/master/LICENSE "bluenotebooks License File ➶"

<!--Badges-->
[lastcommit]:https://img.shields.io/github/last-commit/jupitersinsight/bluenotebooks?style=for-the-badge
[img-pr-badge]:https://img.shields.io/badge/PRs-welcome-orange.svg?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BIDwvZz48L3N2Zz4%3D
[img-license-badge]:https://img.shields.io/badge/license-gnu-367588.svg?style=for-the-badge

